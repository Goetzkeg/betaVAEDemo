...
class BetaVAEv00MaskedModel(pl.LightningModule):  # Conv2d_autoencoder
    ...
    def forward(self, masked_img_tensor, mask_tensor= None): # I do not need the mask tensor here, but in this way I can reuse the plot callbacks easier
        if masked_img_tensor.dim() == 4: #remove color channel if present
            masked_img_tensor = torch.squeeze(masked_img_tensor,1)
        z = self.encoder(masked_img_tensor)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        dict_valid = {
            ...
            'val/disent_loss': loss_disent,
            'val/raw_disent': loss_disent/self.beta
