...
class PConvUNet(nn.Module):
    def __init__(self, channels=3):
        ...
        self.encoder1 = PConvEncoder(channels, 64, 5, bn=False, stride = 1, padding = 10) #out 128
        self.encoder2 = PConvEncoder(64, 128, 5, stride = 1) #out 64
        self.encoder3 = PConvEncoder(128, 256, 3 , stride = 1) #out 32
        self.encoder4 = PConvEncoder(256, 512, 3, stride = 1) #out 16
        self.encoder5 = PConvEncoder(512, 512, 3, stride = 1) #out 8
        self.encoder6 = PConvEncoder(512, 512, 3, stride = 1) #out 4
     ...
     #   self.decoder1 = PConvDecoder(512 + 512, 512, 3)
      #  self.decoder2 = PConvDecoder(512 + 512, 512, 3)
        ...
        self.decoder8 = PConvDecoder(64 + channels, 1, 3, bn=False, act=False, return_mask=False, scale_factor=240/128)
       # self.convfinal = nn.Conv2d(3, 1, 1)
    ...
    def forward(self, inputs_img, inputs_mask):
     ...
     #   e_conv7, e_mask7 = self.encoder7(e_conv6, e_mask6)
      #  e_conv8, e_mask8 = self.encoder8(e_conv7, e_mask7)
        ...
        d_conv9, d_mask9 = self.decoder3(e_conv6, e_mask6, e_conv5, e_mask5)
        d_conv10, d_mask10 = self.decoder4(d_conv9, d_mask9, e_conv4, e_mask4)
        d_conv11, d_mask11 = self.decoder5(d_conv10, d_mask10, e_conv3, e_mask3)
        d_conv12, d_mask12 = self.decoder6(d_conv11, d_mask11, e_conv2, e_mask2)
        d_conv13, d_mask13 = self.decoder7(d_conv12, d_mask12, e_conv1, e_mask1)

        output = self.decoder8(d_conv13, d_mask13, inputs_img, inputs_mask)
