...
class PConvDecoder(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, bn=True, act=True, return_mask=True, scale_factor = 2):
        super(PConvDecoder, self).__init__()
        self.upsample_img = nn.UpsamplingNearest2d(scale_factor=scale_factor)
        self.upsample_mask = nn.UpsamplingNearest2d(scale_factor=scale_factor)

        self.pconv = PartialConv2d(in_channels, out_channels, kernel_size=kernel_size, stride=1,
